...
check "forward-no-href" '! printf "%s" "$PREAUTH_OUT" | sed '"'"'s/href="data:[^"]*"//g'"'"' | grep -qi "href"'
check "status-no-href" '! printf "%s" "$AUTHED_OUT" | sed '"'"'s/href="data:[^"]*"//g'"'"' | grep -qi "href"'
check "status-favicon-icon" 'printf "%s" "$AUTHED_OUT" | grep -q '"'"'rel="icon" type="image/png" href="data:image/png;base64,'"'"''
